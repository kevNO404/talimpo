...
<form action= regcaixaentrada.php method="post">
  ...
  <?php 
       include 'php/conexao.php';
          ...
          $data = $data = implode("-",array_reverse(explode("/",$data)));
           ...
           $sqlconsulta = "SELECT * from detalhecaixa where tipo = 'entrada' and data like '%$data%'";
    ...
    <tr>
      ...
      <td><a href='php/delregcaixaentrada.php?id=<?=$id?>'>Excluir</a></td>
    </tr>
           <?php } 
           }else{

           $sqlconsulta = "SELECT * from detalhecaixa where tipo = 'entrada' order by data desc";
           $consulta = mysqli_query($con, $sqlconsulta);
		       while ($linha = mysqli_fetch_array($consulta)) {
                  $id = $linha["id"]; 
                  $valor = $linha["valor"];
